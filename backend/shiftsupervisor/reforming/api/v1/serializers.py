...
class DailyDataReformingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        def validate(self, data):
            if not (data.get('u100_data') or data.get('u200_data') or data.get('u250_data') or data.get('u300_data') or data.get('u350_data')) :
                raise serializers.ValidationError("حداقل یکی از داده‌ها (Unit100,Unit200,Unit250,Unit300,Unit350) باید وارد شود.")
            return data
